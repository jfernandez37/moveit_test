...
def generate_launch_description():
    ...
    all_params[f"ur_robot_commander"] = {"ros__parameters" : {}}
    # Robot Commander Node
    urdf = os.path.join(get_package_share_directory("aprs_description"), f"urdf/aprs_ur.urdf.xacro")
            
    moveit_config = (
        MoveItConfigsBuilder(f"aprs_ur", package_name=f"aprs_ur_moveit_config")
        .robot_description(file_path=urdf)
        .robot_description_semantic(file_path=f"config/aprs_ur.srdf")
        .trajectory_execution(file_path="config/controllers.yaml")
        .planning_pipelines(pipelines=["ompl"])
        .joint_limits(file_path="config/joint_limits.yaml")
        .moveit_cpp(
            file_path=get_package_share_directory(f"aprs_ur_moveit_config")
    ...
    for k,v in parameters_dict.items():
        all_params[f"ur_robot_commander"]["ros__parameters"][k] = v
    ...
    robot_commander_node = Node(
            package="moveit_test",
            executable="ur_robot_commander_node",
            output="screen",
